 ...
 * Build styles
 */
import { getLineStartPosition } from './editorjs-stringutils';
import './editorjs-code.css';
 ...
 */
export default class HLJSCodeTool {
  ...
  constructor({ data, config, api, readOnly }) {
    ...
    this.placeholder = this.api.i18n.t(config.placeholder || HLJSCodeTool.DEFAULT_PLACEHOLDER);
    this.defaultLanguagesOptions = this.api.i18n.t(config.defaultLanguagesOptions || HLJSCodeTool.DEFAULT_LANGUAGES_OPTIONS);
    this.defaultLanguage = this.api.i18n.t(config.defaultLanguage || HLJSCodeTool.DEFAULT_LANGUAGE);
  ...
  drawView() {
    const wrapper = document.createElement('div'),
      ...
      codeclass = document.createElement('select');


    wrapper.classList.add(this.CSS.baseClass, this.CSS.wrapper);
    textarea.classList.add(this.CSS.textarea, this.CSS.input);
    textarea.setAttribute('spellcheck', false);
    codeclass.classList.add(this.CSS.codeclass);
    textarea.textContent = this.data.code;
    textarea.placeholder = this.placeholder;

    this.defaultLanguagesOptions.forEach(element => {
      const _opt = document.createElement('option');
      _opt.value = element.value;
      _opt.innerText = element.name;
      codeclass.appendChild(_opt);
    });
    codeclass.value = this.data.language;

    if (this.readOnly) {
      textarea.disabled = true;
      codeclass.disabled = true;
    }

    wrapper.appendChild(textarea);
    wrapper.appendChild(codeclass);
    ...
    this.nodes.codeclass = codeclass;
  ...
  onPaste(event) {
    ...
    this.data = {
      code: content.textContent, //getting the entire HTML text content
      language: content.getAttribute('language') || this.defaultLanguage,
    };
  }
  ...
  set data(data) {
    ...
    if (this.nodes.codeclass) {
      this.nodes.codeclass.value = data.language;
    }
  }
  ...
  static get DEFAULT_LANGUAGES_OPTIONS() {
    return [
      {
        name: 'PowerShell',
        value: 'powershell',
